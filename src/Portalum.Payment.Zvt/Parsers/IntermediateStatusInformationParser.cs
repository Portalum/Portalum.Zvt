...
namespace Portalum.Payment.Zvt.Parsers
{
    ...
    public class IntermediateStatusInformationParser
    {
        ...
        public IntermediateStatusInformationParser(ILogger logger)
        {
            this._logger = logger;

            this._statusMessages = new Dictionary<byte, string>()
            {
        ...
        public string GetMessage(Span<byte> data)
        {
            ...
            if (!this._statusMessages.TryGetValue(id, out var message))
            {
